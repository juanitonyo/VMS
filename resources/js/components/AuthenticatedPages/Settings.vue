            ...
            <ul
                ...
                <li class="w-full" v-for="(item, index) in tabs" :key="item.name">
                    <a href="#" @click.prevent="setTab(item.name)"
                        :class="[index == 0 ? 'rounded-l-lg' : (tabs.length - 1 == index ? 'rounded-r-lg' : ''), currentTab == item.name ? 'bg-gray-900 dark:bg-gray-900 text-white ' : 'dark:bg-gray-600 bg-gray-700 text-white', 'inline-block w-full p-4 text-gray-900 bg-gray-900 focus:outline-none dark:text-white']"
                        aria-current="page">
                        {{ item.name }}
                    </a>
                </li>
